      ...
      <ul>
        {todos.map((todo, index) => {
          return(
            <>
            <div key={index}>
          <li  className="bg-white p-2 mb-2 rounded-md shadow">
            {todo}
          </li>
          <button onClick={()=>deleteTodo(index)} className="bg-blue-500 text-white px-4 rounded-r-md hover:bg-blue-600 focus:outline-none focus:bg-blue-600">Delete</button>
          </div></>
)})}
      </ul>
    </div>
