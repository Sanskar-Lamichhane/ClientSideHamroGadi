...
function CustomerDashboard() {
    ...
    useEffect(() => {
        axios.get("http://localhost:3000/api/admin/customerList", {
            headers: {
    ...
    const handleAccountAction=()=>{
        axios.put(`http://localhost:3000/api/admin/user/${confirmationModal.customer._id}/status`,
            {
        ...
        <div className="dashboard p-6 bg-gray-50 font-sans">
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden">
                <div className="p-6 border-b bg-gradient-to-r from-blue-50 to-blue-100 flex justify-between items-center">
                    ...
                    <span className="text-sm text-gray-600 font-medium">Total Customers: {customerList.length}</span>
                </div>
                                ...
                                <tr key={customer._id} className="hover:bg-blue-50 transition-colors">
                                    <td className="p-4">
                                        <div className="flex items-center">
                                            <div className={`p-2 rounded-full mr-3 ${customer.isActive ? 'bg-green-100' : 'bg-red-100'}`}>
                                                <Users className={customer.isActive ? 'text-green-600' : 'text-red-600'} size={20} />
                                            </div>
                                    ...
                                    <td className="p-4 text-right">
                                        {customer.isActive ? (
                                            ...
                                            <button 
                                                onClick={() => openConfirmationModal(customer, 'unlock')}
            ...
            {confirmationModal.show && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg p-6 max-w-sm w-full">
                        ...
                        <p className="mb-6">
                            Are you sure you want to {confirmationModal.action} the account for {confirmationModal.customer.name}?
                        </p>
