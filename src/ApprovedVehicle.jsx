...
function ApprovedVehiclesDashboard() {
    ...
    const fetchApprovedVehicles = () => {
        ...
        axios.post("http://localhost:3000/api/vendor/rentalList",
            ...
            console.log(err)
            setLoading(false);
            toast.error("Failed to load approved vehicles", {
                position: "top-right",
                autoClose: 5000,
        ...
        <div className="dashboard p-6 bg-gray-50 font-sans">
            ...
            <div className={`bg-white rounded-xl shadow-lg ${userRole==="customer"?"container":""} overflow-hidden`}>
                <div className="p-6 border-b bg-gradient-to-r from-blue-50 to-blue-100 flex justify-between items-center">
                    <h2 className="text-xl font-bold text-gray-800">Approved Vehicles</h2>
                    <span className="text-sm text-gray-600 font-medium">Total Approved: {vehiclesList.length}</span>
                </div>
                
                {loading ? (
                    <div className="p-8 text-center text-gray-500">Loading approved vehicles...</div>
                ) : (
...
export default ApprovedVehiclesDashboard;